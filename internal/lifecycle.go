...
func (l *LifeCycle) runLifeCycle(state *lifeCycleState) {
	...
	if l.conf.Partition.State.Failures.Load() != 0 {
	...
	if err := l.conf.Storage.LifeCycleInfo(ctx, &info); err != nil {
		retryIn := retry.DefaultBackOff.Next(int(l.conf.Partition.State.Failures.Add(1)))
...
func (l *LifeCycle) recoverPartition(state *lifeCycleState) bool {
	...
	l.conf.Partition.State.UnReserved.Store(int32(stats.Total - stats.TotalReserved))
	l.conf.Partition.State.Failures.Store(0)
	return true
}
