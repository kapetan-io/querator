...
func (m *MemoryRoleBindings) ListByUser(_ context.Context, userID, namespace string, bindings *[]types.RoleBinding, limit int) error {
	m.mu.RLock()
	defer m.mu.RUnlock()
	...
	for _, id := range ids {
		...
		if b, ok := m.bindings[id]; ok && b.Namespace == namespace {
